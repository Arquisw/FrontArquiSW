...
export class HttpService {
  ...
  public doGetByEmail<T>(serviceUrl: string, correo: string, opts?: Options): Observable<T> {
    const ropts = this.createOptions(opts);
    return this.http.get<T>(serviceUrl + correo, ropts);
  }
  ...
  public doPostWithOutBodyAndId<T>(serviceUrl: string, id: number, opts?: Options): Observable<T> {
    const ropts = this.createOptions(opts);

    return this.http.post<T>(serviceUrl + id, ropts);
  }

  public doRequestMapping<T>(serviceUrl: string, id: number, opts?: Options): Observable<T> {
    const ropts = this.createOptions(opts);
  ...
  public doPutWithOutBody<R>(serviceUrl: string, id: number, opts?: Options): Observable<R> {
    const ropts = this.createOptions(opts);

    return this.http.put<R>(serviceUrl + id, ropts);
  }
